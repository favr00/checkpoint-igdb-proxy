// Vercel serverless function — proxies CheapShark API
// Deploy to: checkpoint-igdb-proxy.vercel.app/api/deals
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!response.ok) {
      return res.status(response.status).json({ error: "CheapShark API error" });
    }
